...
public class OpenSSHKeyV1KeyFile extends BaseFileKeyProvider {
    ...
    private KeyPair readUnencrypted(final PlainBuffer keyBuffer, final PublicKey publicKey) throws IOException, GeneralSecurityException {
        ...
        switch (kt) {
            ...
            case RSA:
                ...
                break;
            case ECDSA256:
                kp = new KeyPair(publicKey, createECDSAPrivateKey(kt, keyBuffer, ECKeySpecFactory.CurveName.SECP256R1));
                break;
            case ECDSA384:
                kp = new KeyPair(publicKey, createECDSAPrivateKey(kt, keyBuffer, ECKeySpecFactory.CurveName.SECP384R1));
                break;
            case ECDSA521:
                kp = new KeyPair(publicKey, createECDSAPrivateKey(kt, keyBuffer, ECKeySpecFactory.CurveName.SECP521R1));
                break;
    ...
    private PrivateKey createECDSAPrivateKey(KeyType kt, PlainBuffer buffer, ECKeySpecFactory.CurveName curveName) throws GeneralSecurityException, Buffer.BufferException {
        kt.readPubKeyFromBuffer(buffer); // Public key
        final BigInteger s = new BigInteger(1, buffer.readBytes());
        final ECPrivateKeySpec privateKeySpec = ECKeySpecFactory.getPrivateKeySpec(s, curveName);
        return SecurityUtils.getKeyFactory(KeyAlgorithm.ECDSA).generatePrivate(privateKeySpec);
    }
